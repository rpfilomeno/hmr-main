...
<head>
	...
	<%
		...
		*/
		...
		AuctionUser auctionUser = request.getAttribute("auctionUser")!=null ? (AuctionUser)request.getAttribute("auctionUser") : (AuctionUser)request.getSession().getAttribute("auctionUser");
        ...
        <section class="page-section color" style="padding:15px;">
            ...
            <div class="container">
                <div class="row" >
                    ...
                    <div class="col-sm-12">
                        <h3 class="block-title"><span>Auction Users  <label>update </label></span></h3>
                        <div id="msgDiv"></div>
 					</div>
               ...
               <div class="form-group">
		              <label><b>Status : </b></label>
						<select class="userStatusSel form-control" id="userStatus" name="userStatus" style="height: 45px; size: 25px;">
								...
								<!-- 
								<option value="1001">For Review</option>
								<option value="1002">Registered</option>
								<option value="1003">Banned</option>
								-->
							<%for(Lov l :auctionUserStatusList){%> 
				...
				<div class="col-sm-12">
					...
					<div class="col-sm-2">
					     <a class="btn btn-theme btn-block " href="#" onclick="clearUpdateAuctionUser()">Clear</a>
					</div>
...
*/
	...
	document.getElementById("auction_id").value="<%=auctionUser.getAuction_id()%>";
	document.getElementById("bidder_id").value="<%=auctionUser.getUser_id()%>";
	document.getElementById("userStatus").value="<%=auctionUser.getStatus()%>";
	document.getElementById("active").value="<%=auctionUser.getActive()%>";

	$(document).ready(function() {
		 $('#auction_id').select2().val(<%=auctionUser.getAuction_id()%>);
		 $('#bidder_id').select2().val(<%=auctionUser.getUser_id()%>);
		 $('#userStatus').select2().val(<%=auctionUser.getStatus()%>);
		 $('#active').select2().val(<%=auctionUser.getActive()%>);
...
function clearUpdateAuctionUser(){
...
function saveUpdateAuctionUser(){
	if(validateSave()){
		document.frm.action.value="saveUpdateAuctionUser";
		document.frm.submit();
	}
