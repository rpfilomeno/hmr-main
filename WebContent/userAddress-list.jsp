...
<head>
	...
	<%
		...
		List<UserAddress> uList =(ArrayList<UserAddress>) request.getAttribute("userAddressList");
    ...
    <div class="content-area">
        ...
        <section class="page-section color" style="padding:15px;">
            <div class="container">
                <div class="row">
                    <div class="col-sm-12">
                        <h3 class="block-title"><span>User Adress  <label>create, list, update, delete </label></span></h3>
 		                ...
 		                <div id="tableDiv1" class="box-body table-responsive">
		                  ...
		                  <table id="table1" class="table table-bordered table-striped no-margin">
		                    ...
		                    <tbody>
                      <%
                      	for(UserAddress u : uList){
                      %>
		                  <tr>
                            <td><a href="#" onclick="view('<%=u.getId()%>')"><%=u.getAddress_line_1()%></a></td>
                            <td><%=u.getBaranggay() %></td>
                            <td><%=u.getCity() %></td>
                            <td><%=u.getCountry() %></td> 
                            <td><%=u.getAddress_type() %></td> 
                            <td><%=u.getPostal_code() %></td>
		                  </tr>    
		             <%} %>         
...
function create(){
	document.getElementById("action").value="createUserAddress";
	document.frm.submit();
}

function view(id){
	document.getElementById("action").value="viewUserAddress";
	document.getElementById("userAddressId_wip").value=id;
	document.frm.submit();
}
...
//resizeTable();
    ...
    <form action="bid" name="frm" id="frm" method="post">
       ...
       <input type="hidden" name="userAddressId_wip" id="userAddressId_wip" value=""/>
    </form>
</body>
</html>