...
<head>
	...
	System.out.println("PAGE item-update.jsp");
		...
		*/
		...
		HashMap<Integer,User> bidderUserHM = (HashMap<Integer,User>)request.getSession().getAttribute("BIDDER-USER-HM");
		             ...
		             <label><b>For Buy : </b></label>
		              ...
		              <div class="form-group">
		                ...
		                <select class="form-control" id="item_increment_time" name="item_increment_time">
								<option value="">ITEM INCREMENT TIME</option>
							<%
							for(int i=0; i < 60; i++){%> 
...
document.getElementById("auction_id").value="<%=item.getAuction_id()%>";
document.getElementById("lot_id").value="<%=item.getLot_id()%>";
document.getElementById("item_id").value="<%=item.getItem_id()%>";
document.getElementById("reference_no").value="<%=item.getReference_no()%>";
document.getElementById("target_price").value="<%=item.getTarget_price()%>";
document.getElementById("reserve_price").value="<%=item.getReserve_price()%>";
document.getElementById("item_desc").value="<%=item.getItem_desc()%>";
document.getElementById("is_buy").value="<%=item.getIs_buy()%>";
document.getElementById("is_bid").value="<%=item.getIs_bid()%>";
document.getElementById("item_increment_time").value="<%=item.getItem_increment_time()%>";
document.getElementById("category_level_1").value="<%=item.getCategory_level_1()%>";
document.getElementById("category_level_2").value="<%=item.getCategory_level_2()%>";
document.getElementById("category_level_3").value="<%=item.getCategory_level_3()%>";
document.getElementById("amount_bid").value="<%=item.getAmount_bid()%>";
...
$(document).ready(function() {
	document.getElementById("bidder_id").value="<%=item.getBidder_id()%>";
	 $('#bidder_id').select2().val(<%=item.getBidder_id()%>);
});
...
function saveItem(){
	if(validateSave()){
		document.frm.action.value="saveUpdateItem";
		document.frm.submit();
	}
