$(document).ready(function() {
	...
	var RTLIssues = {
		...
		post: function () {
			...
			$('.issue-form-wrap .form-control').live('change', function() { 
				somethingChanged = 1;
			});
			...
			$addPostForm.on('submit', function (e) {
				...
				.always(function(data) {
					...
					if ( data.is_valid ) {
						...
						setTimeout(function () {
							window.location = base_url + 'cms/issues';
						}, 1000);
					...
					bootbox.dialog({
					  ...
					  buttons: {
					    ...
					    success: {
					      ...
					      className: "btn-primary btn-sm",
					      callback: function() {
					    	RTLIssues.setDelete($this.data('id'), function (data) {
					    		if ( data.is_valid ) {
					    			webArtJs.messenger({
			...
			RTLIssues.tags();
			RTLIssues.blogCategory();
			RTLIssues.dateTimePicker();
			RTLIssues.postImage();
		...
		setDelete: function(id, callback) {
			...
			return $.ajax({
				url: base_url + 'cms/issues/set_deleted',
				type: 'POST',
				dataType: 'json',
		...
		dateTimePicker: function() {
			...
			RTLIssues.datePostedText(formattedDate);
					  ...
					  buttons: {
					    ...
					    success: {
					      ...
					      callback: function() {
					    	...
					    	RTLIssues.addNewCategory(value, function (data) {
					    		console.log(data)
					    		if ( data.is_valid ) {			
					    			selectProgram = 1;
							      	control.removeOption(value, true)
	  ...
	  //   	var $tags = $('#tags').selectize({
			...
			$tagSearch.kTypeAhead({
				url: base_url + 'cms/issues/tag_suggest',
				data: {
				    type: 'issue_tag',
				    limit: 15,
				},
				...
				onEnter: function () {
				    ...
				    if ( $.inArray($tagSearch.val(), existingTags) == -1 ) {
				    	RTLIssues.addTagItem( $tagSearch.val() )
				  ...
				  buttons: {
				    ...
				    success: {
				      ...
				      callback: function() {
				      	...
				      	if ( $this.data('id') ) {
				      		RTLIssues.deleteTag( $this.data('id'), function(data) {
	    ...
	    deleteTag: function(id, callback) {
			...
			return $.ajax({
				url: base_url + 'cms/issues/delete_tag',
				type: 'POST',
				dataType: 'json',
				data: {is_ajax: 1, id: id},
			})
