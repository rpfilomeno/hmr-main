...
import bizoncloudone.com.manager.LoginManager;
...
public class Controller extends HttpServlet {
	...
	String accessKeyGet = null;
	
	@Override
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
			throws ServletException, IOException {

		emaidAddPost = req.getParameter("emailAdd");
		doProcess(req, resp);
	}
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse res)
		...
		String access_key = req.getParameter("ak")!=null ? (String)req.getParameter("ak") : "";
		String respond = req.getParameter("r")!=null ? (String)req.getParameter("r") : "";
		String sid = req.getSession().getId();
		
		actionGet = action;
		accessKeyGet = access_key;
		
		
		System.out.println("actionGet : "+actionGet);
		System.out.println("accessKeyGet : "+accessKeyGet);
		
		if(actionGet!=null && accessKeyGet!=null && respond!=null){
			
			String page = "";
			
			req.setAttribute("action", action);
			req.setAttribute("access_key", access_key);
			req.setAttribute("respond", respond);
			req.setAttribute("sid_"+action+"_"+access_key, sid);
			System.out.println("");
	...
	protected void doProcess(HttpServletRequest req, HttpServletResponse res)
		...
		String sid = req.getSession().getId();
		String action = req.getParameter("action")!=null ? (String)req.getParameter("action") : (String) req.getSession().getAttribute("userLastAction_"+sid);
		String emailAdd = emaidAddPost;
		String page = null;
		String access_key = req.getParameter("access_key")!=null ? (String)req.getParameter("access_key") : (String) req.getSession().getAttribute("access_key");
		String respond = req.getParameter("respond")!=null ? (String)req.getParameter("respond") : (String) req.getSession().getAttribute("respond");
		...
		System.out.println("Current User : "+emailAdd);

		if("".equals(emailAdd)){
			emailAdd = (String)req.getSession().getAttribute("emailAdd_"+emailAdd);
		}else if(emailAdd==null){
			emailAdd = (String)req.getSession().getAttribute("emailAdd_"+emailAdd);
		}
		
		System.out.println("Current User After 1st Checking : "+emailAdd);
		
		if("".equals(emailAdd)){
			emailAdd = (String)req.getSession().getAttribute("emailAdd_"+sid);
			System.out.println(" sid emailAdd : "+emailAdd);
		}else if(emailAdd==null){
			emailAdd = (String)req.getSession().getAttribute("emailAdd_"+sid);
			System.out.println(" sid emailAdd : "+emailAdd);
		}
			
		System.out.println("Current User After 2nd Checking : "+emailAdd);
			
		req.setAttribute("emailAdd", emailAdd);
		req.setAttribute("sid", sid);
		req.getSession().setAttribute("userLastAction_"+sid, action);
		
		

		if(emailAdd!=null && !"".equals(emailAdd)){
			
			//HashMap hmMngr = new HashMap();
			//hmMngr.put("login","LoginManager"); 817FB70C278B83A45DC835314E1E74D7
			String lastAction = (String)req.getSession().getAttribute("userLastAction_"+sid);
			System.out.println("lastAction :"+lastAction+" "+sid);
			...
			}else if("hmr-login".equals(action) || "reSignIn".equals(action)){
			
			LoginManager liMngr = new LoginManager(req, res);
			
			UserLogin ul = null;
			
			if("hmr-login".equals(action)){
				ul = liMngr.getUserLogin(emailAdd);
			}else{
				ul = (UserLogin)req.getSession().getAttribute("userLogin_"+sid);
